...
export default function AdminArticles() {
  ...
  const fetchArticles = useCallback(async () => {
    try {
      ...
      const response = await authAPI.authenticatedFetch(`/api/articles?${params}`);
  ...
  const handleDeleteArticle = async (id: number) => {
    try {
      const response = await authAPI.authenticatedFetch(`/api/articles/${id}`, {
        method: 'DELETE'
      });
  ...
  const handleBatchDelete = async () => {
    try {
      const promises = selectedArticles.map(id =>
        authAPI.authenticatedFetch(`/api/articles/${id}`, {
          method: 'DELETE'
        })
      );
  ...
  const handleStatusChange = async (id: number, status: string) => {
    try {
      // 先获取文章详情
      const getResponse = await authAPI.authenticatedFetch(`/api/articles/${id}`);
