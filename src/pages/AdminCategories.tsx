...
export default function AdminCategories() {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      setLoading(true);
      const response = await authAPI.authenticatedFetch('/api/categories');
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      setSubmitting(true);
      const response = await authAPI.authenticatedFetch(`/api/categories/${selectedCategory.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          name: formData.name.trim(),
          description: formData.description.trim() || undefined
  ...
  const handleDeleteCategory = async (categoryId: number) => {
    try {
      const response = await authAPI.authenticatedFetch(`/api/categories/${categoryId}`, {
        method: 'DELETE'
      });
      const data = await response.json();
