...
export default function AdminLogs() {
  ...
  const loadLogs = async () => {
    try {
      ...
      const result = await fetchLogs(currentPage, 20, filters);
      if (result) {
  ...
  const handleExportLogs = async () => {
    try {
      ...
      const response = await authAPI.authenticatedFetch(`/api/admin/logs/export?${params}`);
  ...
  const handleCleanOldLogs = async () => {
    ...
    try {
      const response = await authAPI.authenticatedFetch('/api/admin/logs/cleanup', {
        method: 'DELETE',
