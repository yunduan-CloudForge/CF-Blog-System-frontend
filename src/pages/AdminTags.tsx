...
export default function AdminTags() {
  ...
  const fetchTags = useCallback(async () => {
    try {
      setLoading(true);
      const response = await authAPI.authenticatedFetch('/api/tags');
  ...
  const handleUpdateTag = async () => {
    ...
    try {
      setSubmitting(true);
      const response = await authAPI.authenticatedFetch(`/api/tags/${selectedTag.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          name: formData.name.trim(),
          color: formData.color
  ...
  const handleDeleteTag = async (tagId: number) => {
    try {
      const response = await authAPI.authenticatedFetch(`/api/tags/${tagId}`, {
        method: 'DELETE'
      });
      const data = await response.json();
