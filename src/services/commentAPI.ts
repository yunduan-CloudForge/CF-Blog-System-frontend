...
export const commentAPI = {
  ...
  getComments: async (query: CommentQuery): Promise<{ comments: Comment[], pagination: PaginationInfo }> => {
    ...
    const response = await fetch(buildApiUrl(`${API_ENDPOINTS.COMMENTS.LIST}?${params}`));
  ...
  createComment: async (commentData: CommentFormData): Promise<Comment> => {
    const response = await createAuthenticatedRequest(buildApiUrl(API_ENDPOINTS.COMMENTS.CREATE), {
      method: 'POST',
  ...
  replyComment: async (commentId: number, content: string): Promise<Comment> => {
    const response = await createAuthenticatedRequest(buildApiUrl(API_ENDPOINTS.COMMENTS.REPLY(commentId)), {
      method: 'POST',
  ...
  likeComment: async (commentId: number): Promise<{ liked: boolean, likes: number }> => {
    const response = await createAuthenticatedRequest(buildApiUrl(API_ENDPOINTS.COMMENTS.LIKE(commentId)), {
      method: 'POST',
    });
  ...
  deleteComment: async (commentId: number): Promise<void> => {
    const response = await createAuthenticatedRequest(buildApiUrl(API_ENDPOINTS.COMMENTS.DELETE(commentId)), {
      method: 'DELETE',
    });
  ...
  editComment: async (commentId: number, content: string): Promise<Comment> => {
    const response = await createAuthenticatedRequest(buildApiUrl(API_ENDPOINTS.COMMENTS.UPDATE(commentId)), {
      method: 'PUT',
      body: JSON.stringify({ content }),
    });
  ...
  reportComment: async (commentId: number, reason: string, description?: string): Promise<void> => {
    const response = await createAuthenticatedRequest(buildApiUrl(API_ENDPOINTS.COMMENTS.REPORT(commentId)), {
      method: 'POST',
  ...
  getCommentById: async (commentId: number): Promise<Comment> => {
    const response = await fetch(buildApiUrl(API_ENDPOINTS.COMMENTS.DETAIL(commentId)));
    const data = await response.json();
