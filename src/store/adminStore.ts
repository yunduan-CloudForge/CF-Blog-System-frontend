...
export const useAdminStore = create<AdminState>((set, get) => ({
  ...
  fetchPermissions: async () => {
    try {
      const response = await authAPI.authenticatedFetch('/api/admin/permissions');
  ...
  fetchUserPermissions: async () => {
    try {
      const response = await authAPI.authenticatedFetch('/api/admin/user-permissions');
      const data = await response.json();
  ...
  updateRolePermissions: async (role: string, permissions: string[]): Promise<boolean> => {
    try {
      const response = await authAPI.authenticatedFetch(`/api/admin/roles/${role}/permissions`, {
        method: 'PUT',
  ...
  fetchUsers: async (page = 1, limit = 10) => {
    try {
      ...
      const response = await authAPI.authenticatedFetch(`/api/users?page=${page}&limit=${limit}`);
      const data = await response.json();
      
      if (data.success) {
        set({ users: data.data.users });
      } else {
        console.error('API返回失败:', data.message || data.error);
        // 如果是认证失败，可能需要重新登录
  ...
  createUser: async (userData): Promise<boolean> => {
    try {
      const response = await authAPI.authenticatedFetch('/api/users', {
        method: 'POST',
  ...
  updateUser: async (id: number, userData): Promise<boolean> => {
    try {
      const response = await authAPI.authenticatedFetch(`/api/users/${id}`, {
        method: 'PUT',
        body: JSON.stringify(userData),
      });
  ...
  resetUserPassword: async (id: number, newPassword: string): Promise<boolean> => {
    try {
      const response = await authAPI.authenticatedFetch(`/api/users/${id}/password`, {
        method: 'PUT',
  ...
  // 获取操作日志
  fetchLogs: async (page = 1, limit = 20, filters: Record<string, unknown> = {}): Promise<{ total: number; totalPages: number; page: number; limit: number } | null> => {
    try {
      ...
      const response = await authAPI.authenticatedFetch(`/api/admin/logs?${queryParams}`);
      const data = await response.json();
      ...
      if (data.logs) {
        ...
        const mappedLogs = data.logs.map((log: any) => ({
          ...
          userEmail: log.email || 'unknown@example.com',
  ...
  fetchStats: async () => {
    try {
      ...
      const response = await authAPI.authenticatedFetch('/api/admin/stats');
      const data = await response.json();
  ...
  fetchDashboardStats: async () => {
    try {
      ...
      const response = await authAPI.authenticatedFetch('/api/admin/dashboard/stats');
      const data = await response.json();
...
export const adminAPI = {
  ...
  logAction: async (action: string, resource: string, details?: Record<string, unknown>): Promise<void> => {
    try {
      await authAPI.authenticatedFetch('/api/admin/log-action', {
        method: 'POST',
